...
中途改道（升级/降级）、插入新请求、上下文快满、宿主自带计划模式的衔接：读 `methods/routing.md`。
...
| `routing.md` | triage 分级、升降级协议、插入任务、上下文预算、模型分层、宿主计划衔接 | 中途改道；插入请求；上下文快满；分级拿不准 |
...
| 信号 | 路由 |
|---|---|
| bug / 测试失败 / 回归 / 意外行为 | `methods/build.md` 调试节——**先根因，后修复** |
| 同一错误第二次出现 / flaky | `methods/debugging.md` 二击规则 |
| 即将宣称"完成/修好/通过"或 commit/合并/交付 | `methods/verify.md` 出证据；L0 允许一句话收据 |
| diff 大且将合并 / 高风险改动完成 | 建议 `helix review`（`methods/review.md`） |
| "进展如何 / 好了没" | 读 journal 报 status，不重推已验证事实 |
| "卡住了" / 同方案反复失败 / 烦躁信号 | `methods/evolve.md` 停滞诊断 + 脱困 |
| 需求含"优化一下 / 看着办 / 大概"且非 L0 | `methods/clarify.md` 访谈补齐该维度 |
| 改动触碰 schema / 并发 / 权限 / 热路径 / UI | `methods/domains.md` 追加领域人格 |
| 中途要改道 / 被插入新请求 / 上下文快满 | `methods/routing.md` |
...
4. L2 的 clarify 阶段回读 `.helix/learnings.md`（存在时）——上个任务的教训是这个任务的起点；账本里记录的门槛校准用 gate 脚本的 `threshold` 参数落地。
